        ...
        <div className="space-y-5 animate-fade-in">
          <div>
            ...
            <p className="text-white/50 text-sm mb-6">
              Join 847 golfers already giving back
            </p>
          </div>

          <div>
            <label className="block text-xs font-display font-semibold text-white/40 uppercase tracking-widest mb-2">
              Full Name
            </label>
            <input
              {...register("full_name")}
      ...
      {step === 2 && (
        <div className="space-y-6 animate-fade-in">
          <div>
            ...
            <p className="text-white/50 text-sm mb-6">
              Cancel anytime · No lock-in · Demo mode active
            </p>
          </div>
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
              {(charities ?? []).map((c) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setValue("charity_id", charityId === c.id ? null : c.id)
                  }
                  className={cn(
                ...
                >
                  ...
                  {c.is_featured && (
                    <span className="badge-gold text-[10px] py-0.5 flex-shrink-0">
                      Featured
                    </span>
                  )}
                </button>
          ...
          {charityId && (
            <div className="animate-fade-in">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="font-display font-extrabold text-accent text-lg">
                  {charityPct}%
                </span>
              </div>
              <input
                ...
                value={charityPct}
                onChange={(e) =>
                  setValue("charity_percentage", Number(e.target.value))
                }
          ...
          <div className="flex items-start gap-3 bg-gold/5 border border-gold/20 rounded-xl p-4">
            ...
            <div>
              <p className="text-xs font-display font-bold text-gold mb-0.5">
                Demo Mode Active
              </p>
              <p className="text-xs text-white/40 leading-relaxed">
                No payment required. Your subscription activates instantly so
                you can explore the full platform.
              </p>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-bg-primary flex items-center justify-center px-4 py-20">
      <div className="w-full max-w-lg">
        <Link
          href="/"
          className="font-display font-extrabold text-2xl tracking-tight block text-center mb-10"
        >
          Golf<span className="text-accent">Gives</span>
        </Link>

        {/* Note: In your original code, the Step Dots were outside the card. 
            Keep them here if you want them visible during Suspense loading. */}
